...
右侧 SQL 编辑区域上方的下拉列表用于数据库选择，你可以在下拉列表中查看和选择目前 SQL 查询需要使用的数据库。选择数据库后，SQL 语句中可以不包括 dbname 执行，相当于已执行 `use database` 语句。
...
!!! note
    在线SQL编辑器目前不支持事务语句，explain 语句，Load local 和 source 相关语句。 
